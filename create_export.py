...
def _extract_repository_name(repository: str, repository_domain: str) -> str:
    repository_name = repository.split("://")[-1].split(f"{repository_domain}/")[-1].strip("/")
    if repository_name.count("/") > 1:
        repository_name = repository_name.split("/")[0] + "/" + repository_name.split("/")[1]
    return repository_name


def _get_repository_stats(g: Github, gl:Gitlab, repository: str, repository_domain: str) -> dict[str, str]:
    if not repository:
        return {}
    ...
    if repository_domain == "github.com":
        try:
            ...
            repo = g.get_repo(repository_name)
        return {"repository_stars_count": repo.stargazers_count,
        ...
        try:
            ...
            return {}
        return {"repository_stars_count": repo.star_count,
                "repository_forks_count": repo.forks_count,
                }
